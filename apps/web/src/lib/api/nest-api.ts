...
import { normalizeRequestId, REQUEST_ID_HEADER } from '@/lib/api/request-id';
...
export type NestApiRequestInit = RequestInit & {
  /**
...
function getPayloadRequestId(payload: unknown): string | undefined {
  if (!isRecord(payload)) {
    return undefined;
  }

  return typeof payload.requestId === 'string' ? normalizeRequestId(payload.requestId) : undefined;
}
...
async function resolveOutgoingRequestId(outgoingHeaders: Headers): Promise<string> {
  const explicitRequestId = normalizeRequestId(outgoingHeaders.get(REQUEST_ID_HEADER));
  ...
  const incomingRequestId = normalizeRequestId(incomingHeaders.get(REQUEST_ID_HEADER));
...
async function requestNestApi<T>(
  ...
  const { timeoutMs = NEST_API_TIMEOUT_MS, ...requestInit } = init;
  ...
  try {
    return await runWithTimeoutSignal(
      ...
      async (signal) => {
        ...
        const responseRequestId =
          normalizeRequestId(response.headers.get(REQUEST_ID_HEADER)) ??
    ...
    if (error instanceof UpstreamRequestTimeoutError) {
      console.error('Nest API request timed out:', {
    ...
    throw new ApiRequestError('ارتباط با سرویس اصلی برقرار نشد', 503, 'API_UNAVAILABLE', requestId);
  }
}

export async function nestApi<T>(path: string, init: NestApiRequestInit = {}): Promise<T> {
